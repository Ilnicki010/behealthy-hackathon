...
module.exports = {
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Restaurants._attributes)
      .filter(attribute => attribute !== Restaurants.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Restaurants._attributes[attribute].type));
